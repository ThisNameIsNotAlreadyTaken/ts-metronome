...
class Metronome {
        ...
        this.timerWorker.onmessage = e => {
            if (e.data === "tick") {
                while (this.nextNoteTime < this.audioContext.currentTime + this.scheduleAheadTime) {
                    this.scheduleNote({ position: this.current16ThNote, time: this.nextNoteTime });
                    this.nextNote();
                }
            }
    ...
    private nextNote(): void {
        const secondsPerBeat = 60.0 / this.options.tempo; // Notice this picks up the CURRENT tempo value to calculate beat length.
